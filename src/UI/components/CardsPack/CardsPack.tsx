...
import {ICardsPackResponse, ICardsPacks} from "../../../DAL/api/packsAPI";
...
import {PaginationBar} from "../../common/components-common/pagination/Pagination";
...
export const CardsPack = React.memo(() => {
    ...
    const page = useSelector<rootReducers, ICardsPackResponse>(state => state.cardsPack)
    ...
    useEffect(() => {
        ...
        dispatch(getCardsPackTC(page.pageCount, page.page))
    }, [dispatch])
    ...
    return (
        <div className={classes.cardsPack}>
                ...
                <div className={classes.icons}>
                    {
                        searchMode
                            ? <div className={classes.search}>
                                <input type="text" autoFocus onChange={onChangeInputHandler}
                                       onKeyPress={onKeyPressInputHandler}
                                       value={inputValue} onBlur={onBlurInputHandler}/>
                                <button className={classes.searchBtn} onMouseDown={onMouseDownSearchBtnHandler}>Search
                                </button>
                            </div>
                            : <SearchIcon style={{width: '30px', height: '30px', cursor: 'pointer'}}
                                          onClick={onSearchIconHandler}/>
                    }
            ...
            {
                addItemMode && <AddItemWindow title={'Add new module'}
                                              checkboxLabel={'Private Module'}
                                              setAddItemMode={setAddItemMode}/>
            }
            <Table data={data} columns={columns} deleteTableItem={deleteTableItem} editTableItem={editTableItem}/>
            <PaginationBar page={page.page} rows={page.pageCount} totalPage={page.cardPacksTotalCount}/>
        </div>
    )
})